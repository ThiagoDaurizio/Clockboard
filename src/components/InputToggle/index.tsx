...
const CompInputToggle = ( { toggle, set_toggle }:IProps ) => {
  return (
    <div
      onClick={() => set_toggle(!toggle)}
      className={`w-[60px] h-[30px] ${toggle ? 'bg-green-400' : 'bg-gray-400'} rounded-3xl transition-colors duration-200 flex items-center cursor-pointer`}
    >
      <span
